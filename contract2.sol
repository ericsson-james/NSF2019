...
contract SensorContract {
  ...
  constructor() public{
    addSensor(55, 45, "Contract One, Max:55, Min:45, Array Left Shift Then Appendment");
  }
 ...
 // }
  ...
  function setDataToSensorById(int id, int data) public returns (bool){
    ...
    //  currentLength = sensors[id].length

      // The number 10 is being hardcoded and will change for different tests.
      if(sensors.dataArray.length == MaxArrayLength){
        for(uint i = 0; i < MaxArrayLength-1; i++){
          sensors.dataArray[i] = sensors.dataArray[i + uint(1)];
        }
         if(data >= sensors.max || data <= sensors.min){
          sensors.abnormalBehaviors ++;
            sensors.dataArray[MaxArrayLength-1] = data;
            return true;
          }
        else{
            sensors.dataArray[MaxArrayLength-1] = data;
